...
test(IntervalsDates) {
	...
	{
		ModesItem* item = model->modesItems.getEmpty();
		item->set(i, 0, datesStr[i]);
		if(item) model->modesItems.push(item);
	}

	dates.target(0);
	assertFalse(dates.skip());
	...
	for (size_t i = 0; i < 7; i++)
	{
		assertTrue(model->modesItems.remove(i));
	}
}
...
test(modes) {
	...
	for (size_t i = 0; i < 3; i++)
	{
		...
		if (item) {
			item->set(i, flags[i], "");
			model->modesItems.push(item);
		}
	}

	modes.target(0);
	assertFalse(modes.skip());
	assertEqual(modes.evaluate(), 85);

	modes.target(1);
	assertFalse(modes.skip());
	assertEqual(modes.evaluate(), 71);

	modes.target(2);
	assertFalse(modes.skip());
	assertEqual(modes.evaluate(), 100);
}

void fillSensors(int temp,int press,int hum) {
	SensorItem * sens = &model->currentSensor;
	sens->humidity = hum;
...
void fillWeather(int tempMin,int tempMax,int cloudCover,int precipi,
	...
	WeatherData * wea = &model->weather[0];
	wea->temp[0] = tempMin;
	wea->temp[1] = 30;

	WeatherData::Forecast * fore = model->weather[0].getCurrent();
